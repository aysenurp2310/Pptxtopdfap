...
def compress_pptx_images(
    ...
    for slide in prs.slides:
        for shape in _iter_all_shapes(slide.shapes):
            ...
            # GÜVENLİK SİGORTASI: "draft" modu sadece JPEG'de çalışır.
            # PNG/BMP/TIFF gibi formatlarda küçültmeden önce görselin
            # TAMAMI ham piksel olarak decode edilmek zorunda — aşırı
            # yüksek çözünürlüklü (örn. 8000x6000+) bir PNG/ekran
            # görüntüsü/tarama tek başına yüzlerce MB bellek isteyip
            # süreci çökertebilir. Bu formatlarda çok büyük görselleri
            # hiç açmadan atlıyoruz (sıkıştırılmadan kalır, ama süreç
            # çökmez) — bir görseli atlamak, tüm işlemi kaybetmekten
            # çok daha iyi bir tercih.
            MAX_SAFE_PIXELS = 30_000_000  # ~30 megapiksel
            if orig_format != "JPEG" and (orig_w * orig_h) > MAX_SAFE_PIXELS:
                logger.warning(
                    "Aşırı büyük %s görsel (%dx%d) güvenlik için atlanıyor",
                    orig_format, orig_w, orig_h,
                )
                pil_img.close()
                continue
